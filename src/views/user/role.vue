    ...
    methods: {
      initCol () {
        this.columns = [
          ...
          { label: '说明', prop: 'desc', width: '350' },
          { label: '创建时间', prop: 'time', width: '200' },
          { slot: 'operate' }
        ]
      },
      ...
      async onSearch () {
        const res = await this.$http.post('/page/role/role/pageList', {
      ...
      handleDelete (index, row) {
        this.$confirm(`是否确定要删除角色 "${row.name}"`, '提示', {
          ...
          await this.$http.delete(`/rest/role/${row._id}`)
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
      ...
      async submitForm (formName) {
        if (this.dialogId) {
          await this.$http.put(`/rest/role/${this.dialogId}`, this.dialogForm)
        } else {
          await this.$http.post('/rest/role', this.dialogForm)
        }
        this.$message({
          type: 'success',
          message: '保存成功'
